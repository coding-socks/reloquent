...
namespace CodingSocks\Reloquent\Eloquent\Concerns;
...
trait HasAttributes
{
    ...
    public function getAttribute($key)
    {
        ...
        if (array_key_exists($key, $this->attributes) ||
            array_key_exists($key, $this->schema) ||
    ...
    {
    // TODO: rename method
        $this->schema = array_merge($this->schema, $casts);
    ...
    protected function getEnumCastableAttributeValue($key, $value)
    {
        ...
        $castType = $this->getSchema()[$key]['type'];
    ...
    {
        if ($this->isCustomDateTimeCast($this->getSchema()[$key]['type'])) {
            return 'custom_datetime';
        }

        if ($this->isImmutableCustomDateTimeCast($this->getSchema()[$key]['type'])) {
            return 'immutable_custom_datetime';
        }

        if ($this->isDecimalCast($this->getSchema()[$key]['type'])) {
            return 'decimal';
        }

        return trim(strtolower($this->getSchema()[$key]['type']));
    }
    ...
    public function setAttribute($key, $value)
    {
        ...
        if (!is_null($value) && $this->isArrayCastable($key)) {
            $value = $this->castAttributeAsArray($key, $value);
        }
    ...
    {
        $enumClass = $this->getSchema()[$key]['type'];
    ...
    /**
     * Cast the given array to string.
     *
     * @param string $key
     * @param mixed $value
     * @return string
     */
    protected function castAttributeAsArray($key, $value)
    {
        $separator = $this->getSchema()[$key]['separator'] ?? '|';

        return implode($separator, Arr::wrap($value));
    }
    ...
    public function getSchema()
    {
        ...
        if ($this->getIncrementing()) {
            $schema = array_merge($schema, [$this->getKeyName() => ['type' => $this->getKeyType()]]);
        }
    ...
    protected function isClassCastable($key)
    {
        ...
        $castType = $this->parseCasterClass($this->getSchema()[$key]['type']);
    ...
    protected function isClassDeviable($key)
    {
        return $this->isClassCastable($key) &&
            method_exists($castType = $this->parseCasterClass($this->getSchema()[$key]['type']), 'increment') &&
    ...
    public function originalIsEquivalent($key)
    {
        ...
        if ($attribute === $original) {
            ...
            return $this->castAttribute($key, $attribute) ===
                $this->castAttribute($key, $original);
        } elseif ($this->isClassCastable($key) && in_array($this->getSchema()[$key]['type'],
