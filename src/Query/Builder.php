...
class Builder extends BaseBuilder
{
     ...
     *
     * @var \CodingSocks\Reloquent\Connection
     */
    public $connection;
     ...
     *
     * @var \CodingSocks\Reloquent\Query\Grammars\Grammar
     */
    public $grammar;
     ...
     *
     * @var \CodingSocks\Reloquent\Query\Processor
     */
    public $processor;
    ...
    /**
     * The schema of the table which the query is targeting.
     *
     * @var array
     */
    public $schema = [];
    ...
    /**
     * The id which the query is targeting.
     *
     * @var mixed
     */
    public $id;
    ...
    public $operators = [
        '=', '<', '>', '<=', '>=', '<>', '!=',
        'contain', 'contains',
        'match', 'matches', 'matchExactly', 'matchesExactly',
    ];
     ...
     *
     * @param  \Closure|BaseBuilder|\Illuminate\Database\Eloquent\Builder|string  $query
     * @param  string  $as
     * @return $this
     *
    ...
    protected function parseSub($query)
    {
        if ($query instanceof self || $query instanceof EloquentBuilder || $query instanceof Relation) {
            ...
            return [$query->toCommand(), $query->getBindings()];
    ...
    public function where($column, $operator = null, $value = null, $boolean = 'and')
    {
        ...
        // If the schema is null we have to make some assumption according to the type
        // of the value.
        $schema = $this->prepareSchema($value, $operator, $this->schema[$column] ?? null);

        // Now that we are working with just a simple query we can put the elements
        // in our array and add the query binding to our array of bindings that
        // will be bound to each SQL statements when it is finally executed.
        $this->wheres[] = compact(
            'type', 'column', 'operator', 'value', 'boolean', 'schema'
        );
    ...
    /**
     * Prepare the schema for a where clause.
     *
     * @param  string  $value
     * @param  string  $operator
     * @param  array  $schema
     * @return array
     */
    public function prepareSchema($value, $operator, $schema) {
        if (! is_null($schema)) {
            return $schema;
        }

        if (Str::startsWith($operator, 'match')) {
            return ['type' => 'string', 'textSearch' => true];
        }
     ...
     * @return $this
     */
    public function orWhere($column, $operator = null, $value = null, $schema = null)
    {
        [$value, $operator] = $this->prepareValueAndOperator(
            $value, $operator, func_num_args() === 2
        );

        return $this->where($column, $operator, $value, $schema, 'or');
    }
     ...
     *
     * @param  \Closure|\Illuminate\Database\Eloquent\Builder|BaseBuilder|\Illuminate\Database\Query\Expression|string  $column
     ...
     *
     * @param  \Closure|BaseBuilder|\Illuminate\Database\Query\Expression|string|null  $column
     * @param  string  $direction
     * @return $this
     */
     ...
     *
     * @param BaseBuilder|\Closure  $query
     * @param  bool  $all
     * @return $this
     */
    ...
    public function toCommand()
    {
        ...
        if (! is_null($this->id)) {
            return $this->grammar->compileFetch($this);
        }
    ...
    public function get($columns = [])
    {
        return collect($this->onceWithColumns(Arr::wrap($columns), function () {
            return $this->processor->processCommand($this, $this->runCommand(), $this->defaultKeyName());
        }));
    }

    /**
     * Run the query as a "search" statement against the connection.
     *
     * @return array
     */
    protected function runCommand()
    {
        return $this->connection->execute($this->toCommand());
    }
    ...
    protected function runPaginationCountQuery($columns = [])
    {
        if ($this->groups || $this->havings) {
            ...
            return $this->newQuery()
                ->from(new Expression('('.$clone->toCommand().') as '.$this->grammar->wrap('aggregate_table')))
                ->mergeBindings($clone)
    ...
    public function cursor()
    {
        ...
        return new LazyCollection(function () {
            yield from $this->connection->cursor(
                $this->toCommand(), $this->getBindings(), ! $this->useWritePdo
            );
        });
    }
    ...
    public function pluck($column, $key = null)
    {
        ...
        $queryResult = $this->onceWithColumns(
            ...
            function () {
                return $this->processor->processCommand(
                    $this, $this->runCommand()
                );
            }
        );
    ...
    {
        // In Redis we can only check element existence by id.
        if (! is_null($id)) {
            $this->id = $id;
        }

        if (is_null($this->id) || ! empty($this->wheres)) {
            throw new LogicException('This database engine only supports existence check by id.');
        }
     ...
     * @return bool
     */
    public function insert(array $values, $key = null)
    {
    ...
    public function update(array $values)
    {
        ...
        return $this->connection->update($sql, $this->cleanBindings($values));
    }
    ...
    {
        // In Redis we can only check delete elements by id.
        if (! is_null($id)) {
            $this->id = $id;
        }

        if (is_null($this->id) || ! empty($this->wheres)) {
            throw new LogicException('This database engine only supports deletion by id.');
        }

        $this->applyBeforeQueryCallbacks();

        return $this->connection->delete(
            $this->grammar->compileDelete($this), $this->cleanBindings($this->bindings)
        );
    }
    ...
    {
        dump($this->toCommand(), $this->getBindings());
    ...
    public function dd()
    {
        dd($this->toCommand(), $this->getBindings());
    }

    /**
     * Get a new id for items without an id.
     *
     * @return string
     */
